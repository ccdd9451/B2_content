...
public class BodyMecanim : MonoBehaviour
{
    ...
    public class Reference
    {
        ...
        public Transform lowHandPos;
        public Transform highHandPos;
    }
    [SerializeField]
    public Reference Reflist;

    public bool WaitingHandShake;
    ...
    private FullBodyBipedIK fik = null;
    ...
    void Awake()
    {
        ...
        this.fik = this.gameObject.GetComponent<FullBodyBipedIK>();
        this.RegisterWithIK();
    }
    ...
    public IEnumerator<RunStatus> RightHandPick(GameObject obj)
    {
        while (true)
        {
            while (animator.GetFloat("GetObj") < 0.9)
            {
                fik.solver.rightHandEffector.position = obj.transform.position;
                fik.solver.rightHandEffector.positionWeight = animator.GetFloat("RightHandWeight");
                yield return RunStatus.Running;
            }

            obj.GetComponent<Rigidbody>().isKinematic = true;
            obj.transform.parent = Reflist.hand.transform;
            obj.transform.localPosition = Vector3.zero;
            obj.transform.localRotation = Quaternion.identity;

            while (animator.GetFloat("RightHandWeight") > 0.1)
            {
                fik.solver.rightHandEffector.position = obj.transform.position;
                fik.solver.rightHandEffector.positionWeight = animator.GetFloat("RightHandWeight");
                yield return RunStatus.Running;
            }
            fik.solver.rightHandEffector.positionWeight = 0f;
            yield return RunStatus.Success;
        }
    }
    public IEnumerator<RunStatus> RightHandReach(GameObject obj = null, float speed = 1.0f)
    {
        while (true)
        {
            float w = 0.0f;
            if (obj == null) obj = Reflist.lowHandPos.gameObject;
            while (w < 1.0f)
            {
                fik.solver.rightHandEffector.position = obj.transform.position;
                fik.solver.rightHandEffector.positionWeight = w;
                yield return RunStatus.Running;
                w += Time.deltaTime / 0.3f;
            }
            yield return RunStatus.Success;
        }
    }
    public IEnumerator<RunStatus> RightHandMove(GameObject obj)
    {
        while (true)
        {
            float w = 0.0f;
            Vector3 currentHandPos = Reflist.hand.transform.position;
            while (w < 1.0f)
            {
                fik.solver.rightHandEffector.position = Vector3.Lerp(
                    currentHandPos, obj.transform.position, w);
                yield return RunStatus.Running;
    ...
    public IEnumerator<RunStatus> RightHandKeep(GameObject obj)
    {
        while (true)
        {
            fik.solver.rightHandEffector.position = obj.transform.position;
    ...
    public IEnumerator<RunStatus> RightHandReturn()
    {
        while (true)
        {
            float w = 1.0f;
    ...
    public RunStatus RightHandClaim(GameObject obj)
    {
        obj.GetComponent<Rigidbody>().isKinematic = true;
        obj.transform.parent = Reflist.hand.transform;
        obj.transform.localPosition = Vector3.zero;
    ...
    public RunStatus PocketClaim(GameObject obj)
    {
        obj.GetComponent<Rigidbody>().isKinematic = true;
        obj.transform.parent = Reflist.pocket.transform;
        obj.transform.localPosition = Vector3.zero;
        obj.transform.localRotation = Quaternion.identity;
        return RunStatus.Success;
    }
    public RunStatus RightHandRelease(GameObject obj)
    {
        obj.GetComponent<Rigidbody>().isKinematic = true;
